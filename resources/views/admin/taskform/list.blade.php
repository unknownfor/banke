...
<div class="page-bar">
  <ul class="page-breadcrumb">
      ...
      <li>
          <span>{!! trans('labels.breadcrumb.taskformList') !!}</span>
      </li>
  </ul>
</div>
<!-- END PAGE BAR -->
        ...
        <div class="portlet light portlet-fit portlet-datatable bordered">
          <div class="portlet-title">
            <div class="caption">
              ...
              <span class="caption-subject font-dark sbold uppercase">{{trans('labels.taskform.list')}}</span>
            </div>
            <div class="actions">
              <div class="btn-group">
                @permission(config('admin.permissions.taskform.create'))
                <a href="{{url('admin/taskform/create')}}" class="btn btn-success btn-outline btn-circle">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    <thead>
                        <tr role="row" class="heading">
                          <th>#</th>
                          <th width="18%"> {{ trans('labels.taskform.name') }} </th>
                          <th> {{ trans('labels.taskform.seq_no') }} </th>
                          <th> {{ trans('labels.taskform.user_type') }} </th>
                          <th> {{ trans('labels.taskform.status') }} </th>
...
<script type="text/javascript" src="{{asset('backend/js/taskform/list.js')}}"></script>
