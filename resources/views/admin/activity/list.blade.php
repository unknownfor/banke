...
<div class="page-bar">
  <ul class="page-breadcrumb">
      ...
      <li>
          <span>{!! trans('labels.breadcrumb.activityList') !!}</span>
      </li>
  </ul>
</div>
<!-- END PAGE BAR -->
        ...
        <div class="portlet light portlet-fit portlet-datatable bordered">
          <div class="portlet-title">
            <div class="caption">
              ...
              <span class="caption-subject font-dark sbold uppercase">{{trans('labels.activity.list')}}</span>
            </div>
            <div class="actions">
              <div class="btn-group">
                @permission(config('admin.permissions.activity.create'))
                <a href="{{url('admin/activity/create')}}" class="btn btn-success btn-outline btn-circle">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    <thead>
                        <tr role="row" class="heading">
                          <th>#</th>
                          <th width="18%"> {{ trans('labels.activity.title') }} </th>
                          <th> {{ trans('labels.activity.url_type') }} </th>
                          <th> {{ trans('labels.activity.url') }} </th>
                          <th> {{ trans('labels.activity.status') }} </th>
                          <th width="15%"> {{ trans('labels.activity.created_at') }} </th>
...
<script type="text/javascript" src="{{asset('backend/js/activity/list.js')}}"></script>
