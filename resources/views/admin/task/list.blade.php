...
<div class="page-bar">
  <ul class="page-breadcrumb">
      ...
      <li>
          <span>{!! trans('labels.breadcrumb.taskList') !!}</span>
      </li>
  </ul>
</div>
<!-- END PAGE BAR -->
        ...
        <div class="portlet light portlet-fit portlet-datatable bordered">
          <div class="portlet-title">
            <div class="caption">
              ...
              <span class="caption-subject font-dark sbold uppercase">{{trans('labels.task.list')}}</span>
            </div>
            <div class="actions">
              <div class="btn-group">
                @permission(config('admin.permissions.task.create'))
                <a href="{{url('admin/task/create')}}" class="btn btn-success btn-outline btn-circle">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    <thead>
                        <tr role="row" class="heading">
                          <th>#</th>
                          <th width="18%"> {{ trans('labels.task.name') }} </th>
                          <th> {{ trans('labels.task.award_coin') }} </th>
                          <th> {{ trans('labels.task.award_type') }} </th>
                          <th> {{ trans('labels.task.cycle') }} </th>
...
<script type="text/javascript" src="{{asset('backend/js/task/list.js')}}"></script>
