...
<div class="page-bar">
  <ul class="page-breadcrumb">
      ...
      <li>
          <span>{!! trans('labels.breadcrumb.marketingambassadorList') !!}</span>
      </li>
  </ul>
</div>
<!-- END PAGE BAR -->
        ...
        <div class="portlet light portlet-fit portlet-datatable bordered">
          <div class="portlet-title">
            <div class="caption">
              ...
              <span class="caption-subject font-dark sbold uppercase">{{trans('labels.marketingambassador.list')}}</span>
            </div>
            <div class="actions">
              <div class="btn-group">
                @permission(config('admin.permissions.marketingambassador.create'))
                <a href="{{url('admin/marketingambassador/create')}}" class="btn btn-success btn-outline btn-circle">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    <thead>
                        <tr role="row" class="heading">
                          <th>#</th>
                          <th width="18%"> {{ trans('labels.marketingambassador.name') }} </th>
                          <th> {{ trans('labels.marketingambassador.mobile') }} </th>
                          <th> {{ trans('labels.marketingambassador.status') }} </th>
                          <th> {{ trans('labels.marketingambassador.invitor_mobile') }} </th>
                          <th> {{ trans('labels.marketingambassador.award_amount') }} </th>
                          <th width="15%"> {{ trans('labels.marketingambassador.created_at') }} </th>
...
<script type="text/javascript" src="{{asset('backend/js/marketingambassador/list.js')}}"></script>
