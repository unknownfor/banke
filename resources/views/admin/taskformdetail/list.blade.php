...
<div class="page-bar">
  <ul class="page-breadcrumb">
      ...
      <li>
          <span>{!! trans('labels.breadcrumb.taskformdetailList') !!}</span>
      </li>
  </ul>
</div>
<!-- END PAGE BAR -->
        ...
        <div class="portlet light portlet-fit portlet-datatable bordered">
          <div class="portlet-title">
            <div class="caption">
              ...
              <span class="caption-subject font-dark sbold uppercase">{{trans('labels.taskformdetail.list')}}</span>
            </div>
            <div class="actions">
              <div class="btn-group">
                @permission(config('admin.permissions.taskformdetail.create'))
                <a href="{{url('admin/taskformdetail/create')}}" class="btn btn-success btn-outline btn-circle">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    <thead>
                        <tr role="row" class="heading">
                          <th>#</th>
                          <th width="18%"> {{ trans('labels.taskformdetail.name') }} </th>
                          <th width="18%"> {{ trans('labels.taskformdetail.task_type_name') }} </th>
                          <th> {{ trans('labels.taskformdetail.seq_no_name') }} </th>
                          <th> {{ trans('labels.taskformdetail.user_type') }} </th>
                          <th> {{ trans('labels.taskformdetail.award_coin') }} </th>
                          <th> {{ trans('labels.taskformdetail.task_boxes') }} </th>
                          <th> {{ trans('labels.taskformdetail.status') }} </th>
...
<script type="text/javascript" src="{{asset('backend/js/taskformdetail/list.js')}}"></script>
