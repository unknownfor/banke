...
<div class="page-bar">
  <ul class="page-breadcrumb">
      ...
      <li>
          <span>{!! trans('labels.breadcrumb.dictList') !!}</span>
      </li>
  </ul>
</div>
<!-- END PAGE BAR -->
        ...
        <div class="portlet light portlet-fit portlet-datatable bordered">
          <div class="portlet-title">
            <div class="caption">
              ...
              <span class="caption-subject font-dark sbold uppercase">{{trans('labels.dict.list')}}</span>
            </div>
            <div class="actions">
              <div class="btn-group">
                @permission(config('admin.permissions.dict.create'))
                <a href="{{url('admin/dict/create')}}" class="btn btn-success btn-outline btn-circle">
                ...
                <table class="table table-striped table-bordered table-hover table-checkable" id="datatable_ajax">
                    <thead>
                        <tr role="row" class="heading">
                          <th>#</th>
                          <th width="15%"> {{ trans('labels.dict.key') }} </th>
                          <th> {{ trans('labels.dict.value') }} </th>
                          <th> {{ trans('labels.dict.description') }} </th>
                          <th> {{ trans('labels.dict.status') }} </th>
                          <th width="15%"> {{ trans('labels.dict.created_at') }} </th>
                          <th width="15%"> {{ trans('labels.dict.updated_at') }} </th>
                            ...
                            <td>
                              <div class="form-group form-md-line-input">
                                <div class="input-group has-success">
                                    ...
                                    <input type="text" class="form-control form-filter" name="key" placeholder="{{ trans('labels.dict.key') }}">
                              ...
                              <div class="form-group form-md-line-input">
                                <select class="bs-select form-control form-filter" data-show-subtext="true" name="status">
                                  <option value="" data-icon="fa-glass icon-success">状态....</option>
                                    @if(trans('strings.dict'))
                                      @foreach(trans('strings.dict') as $status_key => $status_value)
...
<script type="text/javascript" src="{{asset('backend/js/dict/dict-list.js')}}"></script>
