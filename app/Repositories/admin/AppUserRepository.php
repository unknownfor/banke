...
class AppUserRepository {
							...
							if ($invitor_id > 0) { // && $award_flag) {
								...
								if ($info_obj != null) {
									...
									$message3 = [ 
											'uid' => $user_profile->invitation_uid,
											'title' => '好友认证成功',
											'content' => '您的好友' . $user_profile->mobile . '已经认证成功！平台已奖励您' . $award_amount . '元现金，快去现金钱包里查看吧！',
									...
									if ($bankeTaskUser != null && $bankeTaskUser->count () > 0) {
										...
										$bankeTaskUser->user_id = $invitor_id;
										$bankeTaskUser->source_Id = $user_profile->id;
	...
	public function updateInvitationTableInfo($uid, $targetMobile, $award_amount) {
		$invitation_table = BankeInvitation::where ( [ 
	...
	public static function getUserInLimitTimeByGroup($startTime, $endTime) {
		...
		$user = $user::where ( 'created_at', '>=', getTime ( $startTime ) );
		$user = $user->where ( 'created_at', '<', getTime ( $endTime ) );
		$user = $user->groupBy ( 'date' )->orderBy ( 'date', 'DESC' )->get ( [ 
